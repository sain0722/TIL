...
const HanldleAddProduct = (req, res) => {  // 상품등록
    ...
    if (req.session.auth && req.session.admin) {
        if (body.itemid == '' || datestr == '') {
            ...
            regdate = new Date();
            db.query('INSERT INTO u20_products (itemid, category, maker, pname, modelnum,rdate,price,dcrate,amount,event,pic,comment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)',
                [body.itemid, body.category, body.maker, body.pname, body.modelnum, regdate,
                    body.price, body.dcrate, body.amount, body.event, prodimage, body.comment], (error, results, fields) => {
                    if (error) {
                        htmlstream = fs.readFileSync(__dirname + '/../views/alert.ejs', 'utf8');
                        res.status(562).end(ejs.render(htmlstream, {
                            'title': '알리미',
...
//  -----------------------------------  상품수정기능 -----------------------------------------
// 상품 수정 화면을 출력
const PrintModifyProductForm = (req, res) => {
    console.log("상품 수정 화면 진입");
    let htmlstream = '';
    let htmlstream2 = '';
    let sql_str = '';
    ...
    if (req.session.auth && req.session.admin) {   // 관리자로 로그인된 경우에만 처리한다
        htmlstream = fs.readFileSync(__dirname + '/../views/header.ejs', 'utf8');    // 헤더부분
        htmlstream = htmlstream + fs.readFileSync(__dirname + '/../views/adminbar.ejs', 'utf8');  // 관리자메뉴
        htmlstream = htmlstream + fs.readFileSync(__dirname + '/../views/adminproduct_modify.ejs', 'utf8'); // 괸리자메인화면
        htmlstream = htmlstream + fs.readFileSync(__dirname + '/../views/footer.ejs', 'utf8');  // Footer
        sql_str = "SELECT itemid, category, maker, pname, modelnum, rdate, price, amount from u20_products where itemid = ?"; // 상품조회SQL

        res.writeHead(200, {'Content-Type': 'text/html; charset=utf8'});

        db.query(sql_str, [req.query.itemid], (error, results, fields) => {  // 상품조회 SQL실행
            if (error) {
                res.status(562).end("AdminPrintProd: DB query is failed");
            } else if (results.length <= 0) {  // 조회된 상품이 없다면, 오류메시지 출력
                htmlstream2 = fs.readFileSync(__dirname + '/../views/alert.ejs', 'utf8');
...
const HandleModifyProductForm = (req, res) => {

    let htmlstream = '';
    let body = req.body;

    console.log(body.category, body.maker, body.pname, body.modelnum, body.price, body.amount, body.itemid)
    if (req.session.auth && req.session.admin) {
        
        let sql_str = 'UPDATE u20_products SET category=?, maker=?, pname=?, modelnum=?, price=?, amount=? WHERE itemid=?';
        let params = [body.category, body.maker, body.pname, body.modelnum, body.price, body.amount, body.itemid];
        ...
        db.query(sql_str, params, (error, results, fields) => {
                if (error) {
                    ...
                    console.log("상품 수정에 성공하였으며, DB에 수정된 내용을 저장하였습니다.!");
                    res.redirect('/adminprod/list');
                }
            });
...
router.get('/modify', PrintModifyProductForm);
router.post('/modify', HandleModifyProductForm);
...
const DeleteProduct = (req, res) => {
    ...
    let sql_str = "DELETE FROM u20_products WHERE itemid IN (?)";
    let params = body.checked;
    db.query(sql_str, params, (error, results, fields) => {
        if (error) {
            htmlstream = fs.readFileSync(__dirname + '/../views/alert.ejs', 'utf8');
            res.status(562).end(ejs.render(htmlstream, {
